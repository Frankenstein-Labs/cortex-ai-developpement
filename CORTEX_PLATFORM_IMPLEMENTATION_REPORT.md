...
forming a request. Cloud Control decodes the route segment once before canonical normalization and
rejects malformed encoding, control characters, backslashes, traversal, and database-limit
violations. The server remains the authority: it authenticates sessions, scopes every query by
organization/workspace, enforces roles for writes, and rejects stale versions.
