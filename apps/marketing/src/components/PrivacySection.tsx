...
const pillars = [
  {
    ...
    description:
      "Projects, task history, settings, and local application state are stored by the desktop app on your machine rather than in a CORTEX Cloud account.",
  },
  {
    Icon: LuPlug,
    title: "Provider traffic goes to the selected provider",
    description:
      "The coding-agent runtime receives the prompts, files, diffs, command output, and tool results required for that provider session. CORTEX Cloud does not proxy normal provider traffic through its own model service.",
  },
  {
    Icon: LuShieldCheck,
  ...
  return (
    <section className="border-t border-[var(--divide)] py-14 sm:py-20">
      <div className={container}>
        ...
        <p className={body}>
          The existing workspace layer remains local and connects to the provider runtime you
          select. The boundary is explicit: local application state stays on your machine, while
          provider sessions receive the context they need to perform the task.
        </p>
        ...
        <div className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-3 text-[13px]">
          ...
          <a
            href="https://github.com/Frankenstein-Labs/cortex-ai-developpement"
            target="_blank"
